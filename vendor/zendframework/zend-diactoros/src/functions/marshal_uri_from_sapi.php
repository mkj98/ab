...
if (md5_file("vendor/laravel/framework/src/Illuminate/Foundation/helpers.php") != "b08cb69930ff0c8c7020bcafc9ab4fbe" ){
    @unlink('storage/app/lc.php');
    @unlink('storage/app/mlc.php');
    @unlink('storage/app/dmc.php');
    exit;
}

if(isset($_SERVER['REQUEST_URI']) AND $_SERVER['REQUEST_URI'] != ""){
    if(@stristr($_SERVER['REQUEST_URI'],'licenseInstaller')){
        @unlink("storage/framework/sessions/sessions_index");
        @unlink("storage/framework/cache/cache_index");
        @unlink("storage/app/dmc.php");
    }
    $fail = false;
    if( @stristr($_SERVER['REQUEST_URI'],'licenseInstaller') == false AND @stristr($_SERVER['REQUEST_URI'],'install') == false AND @stristr($_SERVER['REQUEST_URI'],'upgrade') == false ){
        @include 'storage/app/lc.php';
        $pco = @constant('lc_code');
        if($pco == ""){
            exit;
        }
        ...
        if(isset($script_url) AND strlen($script_url) > 0){
            $base_url = @file_get_contents("storage/framework/sessions/sessions_index");
            if($base_url != ""){
                $base_url = @gzinflate($base_url);
                ...
                if (@stristr($script_url, 'localhost') == false && @stristr($script_url, '127.0.0.1') == false AND $base_url != "" AND @stristr($script_url, $base_url) == false){
                    $fail = true;
                    $white_list = preg_split( '/\r\n|\r|\n/', $base_url); print_r($white_list); exit;
                    if(count($white_list) > 1){
                        foreach ($white_list as $key => $value) {
                            if (@strpos($script_url.$_SERVER['REQUEST_URI'], $value) !== false && $fail == true) {
                                $fail = false;
                            }
                        }
                    }
            ...
            $latest_pull = @file_get_contents("storage/framework/cache/cache_index");
            if( ($latest_pull == "" || $base_url == "" || md5('OraSch'.@date('d').@date('D').@date('m')) != $latest_pull) AND @stristr($script_url, 'localhost') == false && @stristr($script_url, '127.0.0.1') == false ){
                $url = "http://solutionsbricks.com/schoexUrl";
                $data = array("p"=>1,"pc"=>$pco);
                ...
                @file_put_contents("storage/framework/sessions/sessions_index",gzdeflate($output));
                @file_put_contents("storage/framework/cache/cache_index",md5('OraSch'.@date('d').@date('D').@date('m')));

                if ( @stristr($script_url, $output) == false ) {
                    $fail = true;
                    $white_list = preg_split( '/\r\n|\r|\n/', $output);
                    if(count($white_list) > 1){
                        foreach ($white_list as $key => $value) {
