...
class invoicesController extends Controller {
	...
	public function listAll($page = 1)
	{
		...
		$toReturn['invoices'] = $toReturn['invoices']->orderBy('id','DESC')->take('20')->skip(20* ($page - 1) )->distinct()->get();
		...
		$classes = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)->distinct()->get();
	...
	public function create(){
		...
		if(\Input::get('invoiceIdGen') == "auto"){
			$feeGroup = \fee_group::where('id',\Input::get('feeGroup'))->first();
			$feeGroup_next_id = $feeGroup->invoice_count;
		}
		...
		foreach($paymentStudent as $key => $value){
			...
			if(\Input::get('invoiceIdGen') == "manual"){
				$payments->paymentTitle = \Input::get('paymentTitle');
			}
			if(\Input::get('invoiceIdGen') == "auto"){
				$feeGroup_next_id = $feeGroup_next_id + 1;
				$payments->paymentTitle = $feeGroup->invoice_prefix . $feeGroup_next_id;
			}
			...
			$std_ids = array();
			$user_list = \User::where('id',$payments->paymentStudent)->select('id','firebase_token')->distinct()->get();
			...
			//Get parents
			if( count($std_ids) > 0 ){
				$user_list = \User::where('role','parent')->where(function($query) use ($std_ids){
					foreach ($std_ids as $key => $value) {
						$query->orWhere('parentOf', 'like', '%"'.$value.'"%');
					}
				})->select('id','firebase_token')->distinct()->get();
		...
		//update fee group
		if(\Input::get('invoiceIdGen') == "auto"){
			\fee_group::where('id',\Input::get('feeGroup'))->update( array( 'invoice_count' => $feeGroup_next_id) );
		}
	...
	function invoice($id){
		...
		if(isset($this->panelInit->settingsArray['paystackEnabled'])){
			$return['paystackEnabled'] = $this->panelInit->settingsArray['paystackEnabled'];
		}
	...
	public function calculate_discounts($invoices_list){
		$available_discount = \fee_discount::where('discount_status','1')->distinct()->get();
		...
		$invoices = $invoices->distinct()->get();
	...
	function collect($id){
		...
		if( floatval(\Input::get('collectionAmount')) > floatval($pendingAmount) ){
	...
	function revert($id){
		...
		//recalculate
		$totalPaid = 0;
		$paymentsCollection = \paymentsCollection::where('invoiceId',$invoice)->distinct()->get();
	...
	public function searchStudents($student){
		$students = \User::where('role','student')->where('fullName','like','%'.$student.'%')->orWhere('username','like','%'.$student.'%')->orWhere('email','like','%'.$student.'%')->distinct()->get();
		$retArray = array();
	...
	function export($type){
		...
		$classes = \classes::select('id','className')->distinct()->get();
		foreach ($classes as $class) {
			$classes_list[$class->id] = $class->className ;
		}

		$sections = \sections::select('id','sectionName','sectionTitle')->distinct()->get();
		...
		if($type == "excel"){
			...
			$data = array(1 => array ('Invoice ID','Title','Student','Class','Section','Amount','Paid Amount','Date','Due Date','Status'));
			...
			$toReturn['totalItems'] = $toReturn['invoices']->count();
			$toReturn['invoices'] = $toReturn['invoices']->orderBy('id','DESC')->distinct()->get();
			...
			foreach ($toReturn['invoices'] as $key => $value) {
				...
				$data[] = array($value->paymentTitle,$value->paymentDescription,$value->fullName,$student_class,$student_section,$return['currency_symbol']." ".$value->paymentAmount,$return['currency_symbol']." ".$value->paidAmount,$value->paymentDate,$value->dueDate,$paymentStatus);
			}
			...
			$toReturn['invoices'] = \DB::table('payments')
						...
						'users.studentSection as studentSection');
			$toReturn['totalItems'] = $toReturn['invoices']->count();
			$toReturn['invoices'] = $toReturn['invoices']->orderBy('id','DESC')->limit('100')->distinct()->get();
